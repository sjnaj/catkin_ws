...
class MoveBaseSeq():
    ...
    def __init__(self):
        ...
        self.P_ar_robot = np.array([0, 0, 0])
    ...
    def callback(self, msg):
        ...
        if(msg.data[len(msg.data)-1] == -100):  # AR模式
            ...
            points_seq = [list(self.P_ros_robot-self.P_ar_robot+msg.data[i:i+3])  # msg.data[i]:P_ar_point
                          for i in range(0, len(msg.data)-3, 3)]

        else:
            points_seq = [list(msg.data[i:i+3]) for i in range(0, len(msg.data), 3)]
    ...
    def callback1(self, msg):
        print(msg.format)
        # vec3_str to vec3
        li = list(filter(None, re.split('[(,)]', msg.format)))

        P_ar_camera = np.array(
            [float(li[0]), float(li[1]), float(li[2])])
        print(P_ar_camera)
        ...
        if ret:    # 画面中有棋盘格
            ...
            print(distance)
            P_camera_robot = tvec*0.01#换算单位为米
            print(P_camera_robot)
            # self.P_ar_robot = P_ar_camera+P_camera_robot#有问题，暂时使用固定值
            self.P_ar_robot = np.array([1, 1, -1]) + P_camera_robot
